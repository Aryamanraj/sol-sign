...
[![npm version](https://badge.fury.io/js/sol-sign.svg)](https://badge.fury.io/js/sol-sign)
...
```bash
sol-sign sign -m "Hello, Solana!" -k ./my-keypair.json
```

### Sign a Message with Private Key

```bash
sol-sign sign -m "Hello, Solana!" -p "your-private-key-here"
```

### Verify a Signature

```bash
sol-sign verify -m "Hello, Solana!" -s "signature-here" -p "public-key-here"
```
...
```bash
sol-sign keypair
sol-sign keypair -o ./new-keypair.json
```
...
sol-sign sign -m "Don\'t forget this" -k ./keypair.json
...
```bash
sol-sign sign -m "Hello World" -k ./keypair.json
sol-sign sign -m "Hello World" -p "your-private-key" -o hex --verify
```
...
**Examples:**
```bash
sol-sign verify -m "Hello World" -s "signature-here" -p "public-key-here"
sol-sign verify -m "Hello World" -s "hex-signature" -p "public-key" -f hex
```
...
Sol-Sign supports three output formats for signatures:
...
Sol-Sign expects Solana keypair files in the standard JSON array format:
...
Sol-Sign provides clear error messages for common issues:
...
git clone https://github.com/Aryamanraj/sol-sign.git
cd sol-sign
...
```typescript
import { SolanaMessageSigner } from 'sol-sign';
